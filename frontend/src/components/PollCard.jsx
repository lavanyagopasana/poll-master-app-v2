...
const PollCard = React.memo(({ poll, onVote, onDelete, hasVoted }) => {
  ...
  const percentages = useMemo(() => {
    ...
    for (const opt of poll.options) {
      map[opt.id] = totalVotesCount === 0
        ? '0.0'
        : ((opt.votes / totalVotesCount) * 100).toFixed(1);
    }
    return map;
  }, [poll.options, totalVotesCount]);

  const badges = useMemo(() => {
    const result = [];
    const hoursOld = Math.abs(Date.now() - new Date(poll.createdAt)) / 36e5;
    if (hoursOld < 24)
      result.push(
    ...
    return result;
  }, [poll.createdAt, totalVotesCount]);
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.02)] flex flex-col h-full hover:shadow-md transition-all duration-500 animate-in fade-in zoom-in-95 slide-in-from-bottom-4">
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <button onClick={() => onDelete(poll.id)} className="text-slate-300 hover:text-red-500 transition-colors p-1">
          <Trash2 size={18} />
        </button>
      </div>
